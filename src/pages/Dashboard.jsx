...
import { Bell, FileText, HeartPulse, PlayCircle, MessageCircle, HelpCircle } from 'lucide-react';
...
const Dashboard = () => {
  ...
  const surveyData = JSON.parse(localStorage.getItem(`netlife_health_survey_${activeProfile?.id}`)) || {
    score: 8,
      ...
      <div className="p-4 md:p-6 bg-gray-50 min-h-screen">
        <header className="flex justify-between items-start mb-6">
          <div className="flex-1">
              <h1 className="text-2xl font-bold text-gray-900">{getGreeting()}, {usernameElement}!</h1>
              <p className="text-gray-500">How are you feeling today? Need care?</p>
            </div>
          <div className="flex justify-end">
            <button onClick={() => navigate('/notifications')} className="relative p-2 rounded-full bg-white border text-gray-600">
              <Bell size={22} />
              {notificationCount > 0 && <span className="absolute top-0 right-0 block h-5 w-5 rounded-full bg-red-500 text-white text-xs flex items-center justify-center ring-2 ring-white">
                  {notificationCount}
                </span>}
            </button>
          </div>
        ...
        <section className="mb-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <button onClick={() => navigate(`/survey/${activeProfile?.id}`)} className="bg-gradient-to-br from-primary to-purple-400 text-white p-4 rounded-2xl text-left flex flex-col justify-between h-32 shadow-lg">
              <FileText size={24} />
              <div>
            ...
            <button onClick={() => navigate('/services')} className="bg-gradient-to-br from-secondary-teal to-teal-400 text-white p-4 rounded-2xl text-left flex flex-col justify-between h-32 shadow-lg">
              <HeartPulse size={24} />
              <div>
        ...
        <section className="mb-6">
          ...
          <div className="flex space-x-4 overflow-x-auto no-scrollbar pb-2">
            <div onClick={() => navigate('/videos')} className="flex-shrink-0 w-40 bg-white border rounded-2xl p-3 cursor-pointer">
        ...
        <section className="mb-6 space-y-3">
          ...
          <div onClick={() => navigate('/history')} className="bg-white border p-4 rounded-2xl flex items-center justify-between cursor-pointer">
            <div>
          ...
          <div onClick={handleFeatureClick} className="bg-white border p-4 rounded-2xl flex items-center justify-between cursor-pointer">
            <div>
        ...
        <section>
          ...
          <div className="bg-white border rounded-2xl p-4 space-y-3">
            <button onClick={() => navigate('/contact-us')} className="flex items-center w-full text-left space-x-3">
              <MessageCircle className="text-primary" />
              <span className="font-semibold text-gray-700">Contact Us</span>
            </button>
            <div className="border-t"></div>
            <button onClick={() => navigate('/faqs')} className="flex items-center w-full text-left space-x-3">
