...
const RecordViewer = () => {
  ...
  const isDbServiceRequest = recordId.startsWith("db_service_request_");
  const isDbScreeningResult = recordId.startsWith("db_screening_result_");

  const dbId = recordId
    .replace("db_service_request_", "")
    .replace("db_screening_result_", "");

  const { data: serviceRequests, isLoading: loadingRequests } =
    useUserServiceRequests(user?.id, { enabled: isDbServiceRequest });
  const { data: screeningResults, isLoading: loadingScreenings } =
    useUserScreeningResults(user?.id, { enabled: isDbScreeningResult });
  const { mutate: deleteServiceRequest } = useDeleteServiceRequest();
  const { mutate: deleteScreeningResult } = useDeleteScreeningResult();

  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);

  const record = React.useMemo(() => {
    if (isDbServiceRequest && serviceRequests) {
      const dbRecord = serviceRequests.find((r) => r.id === dbId);
      if (dbRecord)
        return {
    ...
    if (isDbScreeningResult && screeningResults) {
      const dbRecord = screeningResults.find((r) => r.id === dbId);
      if (dbRecord)
        return {
    ...
    if (!isDbServiceRequest && !isDbScreeningResult) {
    ...
    return null;
  }, [recordId, serviceRequests, screeningResults, activeProfile]);

  const loading = loadingRequests || loadingScreenings;
  ...
  if (!record) {
    return <div className="text-center p-8">Record not found.</div>;
  }
  ...
  if (record.type === "database_service_request") {
    ...
    recordType = "service_request";
    const serviceSlug = recordId.split("_")[2];
    title = serviceRequestForms[serviceSlug]?.title || "Service Request";
    ...
    recordType = "screening_result";
    const serviceSlug = recordId.split("_")[2];
    title = serviceRequestForms[serviceSlug]
      ? `${serviceRequestForms[serviceSlug].title} - Screening`
      : "Service Screening";
  }
  ...
  const renderAvatar = (p) => {
    if (p?.profilePhoto) {
      return (
        <AvatarImage src={p.profilePhoto} alt={p.full_name || p.username} />
      );
    }
    if (p?.avatar) {
      return (
  ...
  const handleDelete = () => {
    ...
    if (recordType === "service_request" && record.data.id) {
      deleteServiceRequest(record.data.id, { onSuccess, onError });
    } else if (recordType === "screening_result" && record.data.id) {
      deleteScreeningResult(record.data.id, { onSuccess, onError });
    } else if (recordId.startsWith("localStorage")) {
      ...
      <div className="space-y-6">
        ...
        {(() => {
          ...
          const recipientName =
            ...
            requestProfile?.full_name ||
            requestProfile?.username ||
            "Account Owner";
          const requestedBy =
            profileInfo?.requestedBy ||
            requestProfile?.full_name ||
            requestProfile?.username ||
            "Account Owner";
              ...
              {!isForMainUser && !isLegacyRecord && (
                <div className="mt-3 p-3 bg-purple-100 rounded-md">
                  <p className="text-sm text-purple-800">
                    <strong>Note:</strong> This service was requested by the
                    account owner for a family member.
                  </p>
                </div>
              )}
              ...
              {isLegacyRecord && (
                <div className="mt-3 p-3 bg-gray-100 rounded-md">
                  <p className="text-sm text-gray-600">
                    <strong>Legacy Record:</strong> This request was made before
                    profile tracking was implemented.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {data.age && (
                <div>
                  ...
                  <p className="text-lg text-gray-800">
                    {safeRenderValue(data.age)} years
                  </p>
                </div>
              )}
            </div>
        ...
        <div className="bg-gray-50 p-3 md:p-4 rounded-lg">
          ...
          <div className="space-y-3">
            {Object.entries(data || {}).map(([key, value]) => {
              ...
              if (skipFields.includes(key) || !value || value instanceof File)
                return null;
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
          ...
          <div className="mt-3 p-3 bg-purple-100 rounded-md">
            <p className="text-sm text-purple-800">
              <strong>Note:</strong> This health risk assessment helps identify
              potential health concerns and provides personalized
              recommendations.
            </p>
          </div>
        </div>
  ...
  const renderScreeningResult = () => {
    const data =
      record.type === "database_screening_result" ? recordData : recordData;
        ...
        {(() => {
          ...
          return (
            <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
              ...
              <div className="mt-3 p-3 bg-purple-100 rounded-md">
                <p className="text-sm text-purple-800">
                  <strong>Note:</strong> This screening determines eligibility
              ...
              {typeof answers === "object" ? (
                Object.entries(answers).map(([questionKey, answer], index) => (
                  ...
                  >
                    <p className="text-sm font-semibold text-gray-500 mb-1">
                      Question{" "}
                      {parseInt(questionKey.replace("question_", "")) + 1 ||
                        index + 1}
                    </p>
                ...
                <div className="bg-white p-4 rounded-lg border border-gray-200">
                  <p className="text-gray-600">
                    Screening answers not available
                  </p>
                </div>
              )}
            </div>
  ...
  const renderEmptyRecord = () => (
    <div className="text-center py-10 px-6 bg-yellow-50 rounded-2xl border border-yellow-200">
      ...
      <p className="mt-2 text-sm text-yellow-600">
        The record you are looking for could not be found. It might have been
              ...
              <div className="flex items-center gap-4 pb-6 border-b">
                ...
                <div>
                  <h2 className="font-bold text-lg text-gray-900">
                    <span className="username-gradient">
                      {record.profile.full_name || record.profile.username}
                    </span>
                  </h2>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                ...
                undone and will permanently remove this record from your
                history.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
