...
const EditableField = ({ field, value, onSave }) => {
    ...
    const isFileAttachment = (val) => {
        return val instanceof File || (typeof val === 'object' && val !== null && (val.path || val.relativePath));
    };
    ...
    const getFileInfo = (val) => {
        ...
        if (val instanceof File) {
            ...
            // Handle legacy file objects with path/relativePath
            fileName = val.path || val.relativePath || 'Unknown file';
            fileUrl = val.path || val.relativePath;
            fileExtension = fileName.split('.').pop()?.toLowerCase() || '';
        }
        ...
        return {
            ...
            isPdf,
            url: fileUrl,
            file: val instanceof File ? val : null
        };
    };
    ...
    useEffect(() => {
        return () => {
            if (fileInfo && fileInfo.url && fileInfo.file) {
    ...
    if (isFileAttachment(value)) {
        return (
            <div className="space-y-3">
                ...
                {/* File preview using AttachmentViewer */}
                <div className="mt-3">
                    <AttachmentViewer
