...
const PreviewStep = ({
  ...
  const userAge = calculateAge(
    activeProfile?.birthDate || activeProfile?.date_of_birth
  );
  ...
  const renderAvatar = (profileData) => {
    ...
    const name =
      (profileData?.full_name || profileData?.username)?.split(" ")[0] || "";
    return (
    ...
    >
      <div className="p-4 bg-white rounded-xl border-2 border-gray-100">
        <div className="flex items-center gap-4">
          <Avatar className="h-16 w-16">{renderAvatar(profile)}</Avatar>
          <div>
            <h3 className="font-bold text-lg text-gray-900">Account Owner</h3>
            <p className="text-sm text-gray-500">
              Main account holder submitting this request.
            </p>
          </div>
        </div>
        <div className="mt-4 space-y-3 text-base text-gray-700">
          <p>
            <strong>Name:</strong> {profile?.full_name || profile?.username}
          </p>
          <p>
            <strong>Phone:</strong>{" "}
            {profile?.phoneNumber || profile?.whatsapp_number}
          </p>
          <p>
      ...
      {isRequestingForOther && (
        <div className="p-4 bg-blue-50 rounded-xl border-2 border-blue-200">
          <div className="flex items-center gap-4">
            <Avatar className="h-16 w-16">{renderAvatar(activeProfile)}</Avatar>
            <div>
              <h3 className="font-bold text-lg text-blue-900">
                Service Recipient
              </h3>
              <p className="text-sm text-blue-600">
                Person receiving this service.
              </p>
            </div>
          </div>
          <div className="mt-4 space-y-3 text-base text-blue-800">
            ...
            {userAge && (
              <p>
                <strong>Age:</strong> {userAge} years
              </p>
            )}
            <p>
      ...
      {!isRequestingForOther && (
        <div className="p-4 bg-white rounded-xl border-2 border-gray-100">
          ...
          <div className="mt-4 space-y-3 text-base text-gray-700">
            ...
            <p>
              <strong>Phone:</strong>{" "}
              {activeProfile?.phoneNumber || activeProfile?.whatsapp_number}
            </p>
            <p>
              <strong>Location:</strong>{" "}
      ...
      {formConfig.steps.map((stepConfig, index) => (
        <div
          key={index}
        ...
        >
          <div className="flex justify-between items-center mb-2">
            ...
            <Button variant="ghost" size="sm" onClick={() => goToStep(index)}>
              <Edit className="h-4 w-4 mr-2" /> Go back & Edit
            </Button>
          </div>
                ...
                return (
                  ...
                  >
                    ...
                    <EditableField
                      ...
                      onSave={(newValue) =>
                        handleFieldSave(field.name, newValue)
                      }
                    />
                  </div>
                );
      ...
      <div className="mt-6 p-4 bg-primary/5 rounded-lg">
        <div className="flex items-start space-x-3">
          ...
          >
            ...
            NetLife processing this request.
          </label>
        </div>
      </div>
      {submitError && (
        <div className="mt-4 p-4 bg-red-50 border border-red-200 rounded-lg">
          ...
          <p className="text-xs text-red-600 mt-1">
            Your request has been saved locally and you can try submitting again
            later.
          </p>
        </div>
      )}
