...
async function _processAttachment(file, userId, options = {}) {
  ...
  try {
    ...
    if (typeof file === "string" && file.startsWith("http")) {
      if (validateUrl) {
        try {
          ...
          logError(urlError, "_processAttachment", {
            userId,
            action: "URL validation failed",
            url: file,
          });
    ...
    try {
      ...
      logError(validationError, "_processAttachment", {
        userId,
    ...
    if (uploadedFilePath) {
      try {
        ...
        logError(cleanupError, "_processAttachment cleanup", {
          userId,
...
async function _cleanupUploadedFile(filePath) {
  try {
    ...
    logError(null, "_cleanupUploadedFile", {
      action: "File cleaned up",
      filePath,
    });
  } catch (error) {
    logError(error, "_cleanupUploadedFile", {
...
async function _cleanupMultipleFiles(filePaths, userId) {
  ...
  try {
    ...
    if (error) {
      ...
      logError(error, "_cleanupMultipleFiles", {
        userId,
        filePaths,
        action: "Batch cleanup failed",
      });
    } else {
      results.success = filePaths.length;
      logError(null, "_cleanupMultipleFiles", {
        userId,
        filePaths,
        action: "Batch cleanup successful",
      });
    }
  } catch (error) {
    results.failed = filePaths.length;
...
async function _cleanupFailedRequestAttachment(attachmentUrl, userId) {
  ...
  try {
    ...
    logError(error, "_cleanupFailedRequestAttachment", {
      userId,
      attachmentUrl,
    });
    return false;
...
async function _cleanupOrphanedFiles(userId, olderThanHours = 24) {
  try {
    ...
    if (dbError) {
      logError(dbError, "_cleanupOrphanedFiles", {
        userId,
        action: "Failed to fetch service requests",
      });
    ...
    const orphanedPaths = orphanedFiles.map(
      (file) => `user_${userId}/${file.name}`
    );
...
export const servicesApi = {
  ...
  async getServiceQuestions(serviceId) {
    try {
      ...
      return await retryWithBackoff(async () => {
        ...
        if (error) {
          throw new Error(
            `Failed to fetch service questions: ${error.message}`
          );
        }
  ...
  async submitServiceRequest(request) {
    ...
    try {
      ...
      _validateAttachmentResultConsistency(
        "submitServiceRequest",
        attachmentResult
      );
      _handleAttachmentResult(
        attachmentResult,
        request.user_id,
        "servicesApi.submitServiceRequest"
      );
      ...
      const requestData = {
        ...
        status: request.status || "pending",
        request_data: request.request_data,
        attachments: attachmentResult.url,
        delivery_method: extractedFields.delivery_method,
        delivery_location: extractedFields.delivery_location,
        preferred_date: extractedFields.preferred_date,
        quantity: extractedFields.quantity,
      ...
      await _handleFailedSubmissionCleanup(
        attachmentResult,
        request.user_id,
        "servicesApi.submitServiceRequest"
      );
  ...
  async deleteServiceRequest(requestId) {
    try {
      ...
      if (fetchError && fetchError.code !== "PGRST116") {
        throw new Error(
          `Failed to verify service request: ${fetchError.message}`
        );
      }
  ...
  async deleteScreeningResult(resultId) {
    try {
      ...
      if (fetchError && fetchError.code !== "PGRST116") {
        throw new Error(
          `Failed to fetch screening result: ${fetchError.message}`
        );
      }
    ...
    try {
      ...
      if (isSync && request.request_data) {
        ...
        const deliveryMethod =
          requestData.deliveryMethod || requestData.accessPoint;
        const locationBasedMethods = [
          "Home Delivery",
          "Community Group Delivery",
        ];

        if (
          locationBasedMethods.includes(deliveryMethod) &&
          !requestData.deliveryLocation
        ) {
          requestData.deliveryLocation =
            requestData.address || "Location to be confirmed";
        }
        if (!requestData.deliveryMethod && !requestData.accessPoint) {
          requestData.deliveryMethod = "Facility pickup";
        }
      ...
      logError(error, "servicesApi.submitServiceRequestForSync", {
        request,
        isSync,
      });
      throw new Error(handleApiError(error));
  ...
  async deleteUserScreeningAnswers(userId, serviceId) {
    try {
      ...
      logError(error, "servicesApi.deleteUserScreeningAnswers", {
        userId,
        serviceId,
      });
      throw new Error(handleApiError(error));
  ...
  async notifyNewVideo(videoTitle, videoId) {
    try {
      ...
      logError(null, "servicesApi.notifyNewVideo", {
        ...
        videoId,
        action:
          "Video notification will be created automatically by database trigger",
      });
  ...
  async getRecentServiceRequests(userId, limit = 2) {
    try {
      ...
      return await retryWithBackoff(async () => {
        ...
        if (error) {
          throw new Error(
            `Failed to fetch recent service requests: ${error.message}`
          );
        }
      ...
      logError(error, "servicesApi.getRecentServiceRequests", {
        userId,
        limit,
      });
      throw new Error(handleApiError(error));
  ...
  async getServiceQuestionsWithOptions(serviceId) {
    try {
      ...
      logError(error, "servicesApi.getServiceQuestionsWithOptions", {
        serviceId,
      });
      throw new Error(handleApiError(error));
    }
  ...
  async cleanupOrphanedAttachments(userId, olderThanHours = 24) {
    try {
      ...
      logError(error, "servicesApi.cleanupOrphanedAttachments", {
        userId,
        olderThanHours,
      });
      throw new Error(handleApiError(error));
  ...
  async getAttachmentStats(userId) {
    try {
      ...
      if (files && files.length > 0) {
        ...
        stats.newestFile =
          sortedFiles[sortedFiles.length - 1]?.created_at || null;
      }
...
export const ATTACHMENT_PROCESSING_CONFIGS = {
  ...
  SYNC: {
    ...
    includeMetadata: false,
    description:
      "Sync service request attachment processing with enhanced retry",
  },
 ...
 */
async function _processAttachmentStandardized(
  file,
  userId,
  configType = "STANDARD",
  overrides = {}
 ...
 * @returns {Promise<void>}
 */
async function _handleFailedSubmissionCleanup(
  attachmentResult,
  userId,
  operation
) {
...
function _validateAttachmentResultConsistency(methodName, attachmentResult) {
  const isValid =
    ...
    (attachmentResult.url === null ||
      typeof attachmentResult.url === "string") &&
    (attachmentResult.error === undefined ||
      typeof attachmentResult.error === "string") &&
    (attachmentResult.metadata === null ||
      typeof attachmentResult.metadata === "object");
