...
export const surveyService = {
  /**
   * Get the survey status for a user. Throws an error on failure.
  ...
  async getSurveyStatus(userId) {
    try {
      ...
      if (error) {
        // Let TanStack Query handle this error
        throw error;
      }
      ...
      const nextAvailableAt = new Date(
        lastCompletedAt.getTime() + threeMonthsInMs
      );
      ...
      // Re-throw the error so TanStack Query can catch it
      throw new Error(error.message || "Failed to fetch survey status.");
    }
  },
  ...
  getNextAvailableMessage(nextAvailableAt) {
    ...
    if (diffInMs <= 0) {
      ...
      return `Available in ${diffInSeconds} second${
        diffInSeconds === 1 ? "" : "s"
      }`;
    } else if (diffInMinutes < 60) {
      return `Available in ${diffInMinutes} minute${
        diffInMinutes === 1 ? "" : "s"
      }`;
